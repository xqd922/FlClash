...
- 内核全面升级到上游 v0.8.96(mihomo `c27f82fb → 0f7f05ad`),支持最新协议与修复
- 产品层切换到上游全新的 action-provider 架构,后续版本可直接跟随上游演进
- 桌面端采用新的 helper 服务体系(核心校验、RPC 通道),服务名保持 `XlclashHelperService`
- Android 适配上游新的 VpnService/ManagedService 架构与 TV 支持

### ♻️ 重构

- 全部本地定制已移植到新架构:HCT 自定义色板、内置主题色、色块 hex 显示
- 仪表盘保持旧版简洁布局(固定布局、无编辑入口、无核心状态按钮)
- 「请求记录」开关保留(默认隐藏),工具页无免责声明入口
- 生成的 mihomo 配置:key 固定顺序输出、剔除 null 字段、配置文件值优先
- 停止代理时自动关闭 external-controller;后台挂起时不执行订阅自动更新
- 仅 VPN 状态真正切换时才刷新 IP 检测
- 移除上游重新带入的纯黑模式、文字缩放与配色方案变体入口(延续 v7.0.0 精简)

### 🔧 修复

- 修正网络切换时 IP 刷新的触发条件(上游逻辑反了)

### 🧪 测试

- 引入上游全量测试套件(714 项全部通过),并适配 Xlclash 默认值与 Windows 环境

## v(7.0.33)

### ✨ 新功能

- 应用名称全面统一为 `Xlclash`，同步更新各平台窗口、通知、VPN 会话、Core、Helper 和安装包名称
- 新增 `xlclash://` 配置链接协议，同时保留旧 `flclash://` 协议以兼容已有链接

### 🔧 修复

- 发布下载链接与实际生成的 `Xlclash-*` 构建产物名称保持一致

## v(7.0.32)

### ✨ 新功能

- 内置 `#35618E` 和 `#67568F` 主题色
